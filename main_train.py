...
import attack
from models import *
from utils import forward_function 
...
from stable_modules_TV_l1 import Lyapunov_Function
import numpy as np
...
parser.add_argument('-lr_p', '--lr_p', default=0.01, type=float, metavar='LR', help='initial learning rate')
parser.add_argument('-dev', '--device', default='0', type=str, help='device')

# adv training configuration
parser.add_argument('-training_mode', '--training_mode', default='Gaussian', type=str,
                    help='[Raw, Gaussian, AT_FGSM, RAT_FGSM]')
parser.add_argument('-fix_p', '--fix_p', default='no', type=str, help='[yes no]')
parser.add_argument('-beta', '--beta', default=0.0010, type=float, help='regulation beta')

parser.add_argument('-kappa', '--kappa', default=0.5, type=float, help='ratio of clean training')
parser.add_argument('-GN', '--GNnoise', default=10, type=float, metavar='N', help='GN noise budget')
parser.add_argument('-alpha', '--alpha', default=1.0, type=float, metavar='N',help='lynapunov perturb data alpha')
parser.add_argument('-eps', '--eps', default=6, type=float, metavar='N', help='attack eps')
...
def main():
    ...
    log_dir = '%s-checkpoints_l1' % (args.dataset)
    ...
    lyapunov_function = Lyapunov_Function('l1')

    best_acc = 0
    identifier = args.model
    identifier += args.neuron_type[:3]
    ...
    if args.training_mode.lower() == 'raw':
        ...
        identifier += '_ATFGSM[%.4f]' % (
        args.alpha)
    elif args.training_mode.lower() == 'rat_fgsm':
        identifier += '_RATFGSM[%.4f]' % (
        args.alpha)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (images, labels) in enumerate(tqdm(train_loader)):
            ...
            if 'fgsm' in args.training_mode.lower():
                ...
                images_adv = images.clone().detach() + args.GNnoise / 255 * torch.randn(images.shape).detach().to(
                    device)
            ...
            if args.training_mode.lower() == 'raw':
                ...
                current_batch_grad_norm = total_norm ** 0.5
                grad_norm_history.append(current_batch_grad_norm)
            ...
            if args.training_mode.lower() == 'gaussian' or 'fgsm' in args.training_mode.lower():
                ...
                with autocast('cuda'):
                    ...
                    loss = criterion(clean_outputs.mean(0), labels) * args.kappa + (1 - args.kappa) * criterion(
                        adv_outputs.mean(0), labels)

                    Y, fx = simple_dynamic(clean_neuron_input - neuron_input['last'], tau=args.tau)
                    V = lyapunov_function(Y)
                    loss_V = V * args.alpha
        ...
        if best_acc < val_acc:
            best_acc = val_acc
            best_model_path = os.path.join(log_dir, '%s_best.pth' % (identifier))
            logger.info(f"--- Saving new best model to {best_model_path} with acc {best_acc:.3f}% ---")
            torch.save({
                'epoch': epoch,
        ...
        save_period = 10
        if (epoch + 1) % save_period == 0 or (epoch + 1) == args.epochs:
            checkpoint_path = os.path.join(log_dir, f'{identifier}_checkpoint_epoch_{epoch + 1}.pth')
            logger.info(f"--- Saving periodic checkpoint to {checkpoint_path} ---")
            torch.save({
                'epoch': epoch,
                'model_state_dict': model.state_dict(),
                'optimizer_state_dict': optimizer.state_dict(),
    ...
    grad_norm_save_path = os.path.join(log_dir, f'{identifier}_grad_norm_history.npy')
    logger.info(f"--- Saving gradient norm history (total {len(grad_norm_history)} steps) to {grad_norm_save_path} ---")
    np.save(grad_norm_save_path, np.array(grad_norm_history))
    logger.info("--- Gradient norm history saved. ---")
