...
class ExpandTemporalDim(nn.Module):
    ...
    def build_neuron_function(type='LIFSpike', **kwargs):
        return neuron.LIFNode(tau=kwargs.get('tau', 1.0), v_threshold=kwargs.get('thresh', 1.0), v_reset=0.0)
...
class ZIF(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        tmp = (1 / gama) * (1 / gama) * torch.nn.functional.softplus((gama - input.abs()),beta=5)
...
class DLIFSpike(nn.Module):
    def __init__(self, T, thresh=1.0, tau=0.99, gama=1.0):
        ...
        self.act = torch._dynamo.disable(ZIF.apply)
        self.thresh = thresh
        self.tau = tau
        self.gama = gama
